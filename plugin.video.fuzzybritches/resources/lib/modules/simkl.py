...
simklclientid = '5cae3f275f1b3add1e9d2f102899c5998b263cab942eebd3c22c9284213f23b7'
...
class SIMKL:
		...
		if response.status_code == 200:
			...
			if responseJson.get('result') == 'KO':
				...
				try:
					...
					if fromSettings == 1:
						control.openSettings('5.2', 'plugin.video.fuzzybritches')
				return
		else:
			log_utils.error()
			return
	...
	def get_account_info(self, token=None):
		try:
			...
			headers['User-Agent'] = 'FuzzyBritches/%s' % control.addon('plugin.video.fuzzybritches').getAddonInfo('version')
...
def get_request(url):
	...
	try:
		...
		_version = control.addon('plugin.video.fuzzybritches').getAddonInfo('version')
		if '?' not in url:
			url += '?client_id=%s&app-name=fuzzybritches&app-version=%s' % (simklclientid, _version)
		else:
			url += '&client_id=%s&app-name=fuzzybritches&app-version=%s' % (simklclientid, _version)
		headers['Authorization'] = 'Bearer %s' % getSetting('simkltoken')
		headers['simkl-api-key'] = simklclientid
		headers['User-Agent'] = 'FuzzyBritches/%s' % _version
...
def scrobbleReset(imdb, tmdb='', tvdb='', season=None, episode=None, refresh=False, clear_local=True):
	...
	try:
		...
		delete_headers = {
			...
			'User-Agent': 'FuzzyBritches/%s' % _version
		}
