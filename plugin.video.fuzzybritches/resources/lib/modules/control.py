...
def getFuzzyBritchesVersion():
	return xbmcaddon.Addon('plugin.video.fuzzybritches').getAddonInfo('version')
...
def skin_location():
	return transPath('special://home/addons/plugin.video.fuzzybritches')
...
def refresh_debugReversed(): # called from service "onSettingsChanged" to clear fuzzybritches.log if setting to reverse has been changed
	if homeWindow.getProperty('fuzzybritches.debug.reversed') != setting('debug.reversed'):
		...
		execute('RunPlugin(plugin://plugin.video.fuzzybritches/?action=tools_clearLogFile)')
...
def disable_enable_addon():
    ...
    jsonrpc('{"jsonrpc": "2.0", "id":1, "method": "Addons.SetAddonEnabled", "params": {"addonid": "plugin.video.fuzzybritches", "enabled": false }}')
    xbmc.log('[ plugin.video.fuzzybritches ] fuzzybritches disabled', 1)
    jsonrpc('{"jsonrpc": "2.0", "id":1, "method": "Addons.SetAddonEnabled", "params": {"addonid": "plugin.video.fuzzybritches", "enabled": true }}')
    xbmc.log('[ plugin.video.fuzzybritches ] fuzzybritches re-enabled', 1)
...
def backToMain(folder):
	if folder == 'movies':
		url = 'plugin://plugin.video.fuzzybritches/?action=movieNavigator&folderName=Discover%20Movies'
	elif folder =='tvshows':
		url = 'plugin://plugin.video.fuzzybritches/?action=tvNavigator&folderName=Discover%20TV%20Shows'
	else:
		url = None
