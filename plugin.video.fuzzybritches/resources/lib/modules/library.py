	...
	def importListsManager(self, fromSettings=False):
		try:
			...
			if fromSettings == True:
				control.openSettings('13.2', 'plugin.video.fuzzybritches')
	...
	def strmFile(self, i):
		try:
			...
			if control.setting('library.strm.use_tmdbhelper') == 'true':
				...
				content = 'plugin://plugin.video.fuzzybritches/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s' % (systitle, year, imdb, tmdb)
	...
	def strmFile(self, i):
		try:
			...
			if control.setting('library.strm.use_tmdbhelper') == 'true':
				...
				content = 'plugin://plugin.video.fuzzybritches/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s' % (
		...
		try:
			...
			for file in episode:
				try:
					...
					read = file.read()
					file.close()
					if not read.startswith(('plugin://plugin.video.themoviedb.helper', 'plugin://plugin.video.fuzzybritches')): continue
