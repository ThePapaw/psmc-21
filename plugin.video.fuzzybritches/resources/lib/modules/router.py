...
def router(argv2):
	...
	if action is None:
		...
		isUpdate = control.homeWindow.getProperty('fuzzybritches.updated')
		if isUpdate == 'true':
			control.execute('RunPlugin(plugin://plugin.video.fuzzybritches/?action=tools_cleanSettings)')
			control.homeWindow.clearProperty('fuzzybritches.updated')
	...
	####################################################
		...
		if action == 'tools_openSubsRevoke':
			...
			if query == 'EasyNews':
				...
				control.sleep(100)
				control.openSettings('11.3', 'plugin.video.fuzzybritches')
				...
				control.syncAccounts()
				control.sleep(100)
				control.openSettings('11.4', 'plugin.video.fuzzybritches')
			elif query == 'Plex':
				...
				control.syncAccounts()
				control.sleep(100)
				control.openSettings('11.7', 'plugin.video.fuzzybritches')
			else:
			...
			elif cleared: control.notification(message='FuzzyBritches Log File Successfully Cleared')
			else: control.notification(message='Error clearing FuzzyBritches Log File, see kodi.log for more info')
			...
			tools.nonsense()
		elif action == 'tools_fuzzybritchesExternalProvider':
			from resources.lib.modules import tools
	...
	####################################################
		...
		if action == 'play_Item':
			...
			for i in items:
				...
				url = 'plugin://plugin.video.fuzzybritches/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select=1' % (
			...
			if rtype == 'movie':
				...
				r = 'plugin://plugin.video.fuzzybritches/?action=play_Item'
			elif rtype == 'episode':
				from resources.lib.menus import episodes
				...
				r = 'plugin://plugin.video.fuzzybritches/?action=play_Random&rtype=episode'
			elif rtype == 'show':
				from resources.lib.menus import tvshows
				rlist = tvshows.TVshows().get(url, create_directory=False)
				r = 'plugin://plugin.video.fuzzybritches/?action=play_Random&rtype=season'
	...
	####################################################
		...
		control.openSettings('0.2', 'plugin.video.fuzzybritches')
	...
	####################################################
		...
		elif action == 'cache_clearKodiBookmark': # context.fuzzybritches action call only
