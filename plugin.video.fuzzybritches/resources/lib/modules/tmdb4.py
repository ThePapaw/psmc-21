# -*- coding: utf-8 -*-
"""
	FuzzyBritches Add-on - TMDB v4 API Support
"""
...
def authenticate(fromSettings=0):
	...
	try:
		...
		if not read_token:
			...
			if fromSettings:
				control.openSettings('8.3', 'plugin.video.fuzzybritches')
			return
		...
		# Always try the fuzzybritches dialog for auth — QR is essential for TV use.
		# Fall back to standard Kodi dialog only if the fuzzybritches dialog fails.
		try:
...
def get_watchlist_sort_by(mediatype='movie'):
	...
	try:
		import xbmcaddon
		_addon = xbmcaddon.Addon('plugin.video.fuzzybritches')
