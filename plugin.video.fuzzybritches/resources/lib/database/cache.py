...
def cache_clear_thumbnails():
	...
	try:
		...
		if fanart is not None:
			...
			dbcur.execute("DELETE FROM texture WHERE url ='" + addonPath + "/plugin.video.fuzzybritches/fanart.jpg';") #delete the fanart
		dbcon.close()
		cleared = True
	except:
		from resources.lib.modules import log_utils
...
def get_video_database_path():
	...
	try:
		...
		#from resources.lib.modules import log_utils
		#log_utils.log('FuzzyBritches MyVideos file path: %s' % str(database_path_final), 1)
		databaseFound = True
	except:
		#from resources.lib.modules import log_utils
		#log_utils.log('FuzzyBritches MyVideos file path exception manual selection needed', 1)
		databaseFound = False
...
def update_cache_version():
	...
	try:
		...
		from resources.lib.modules import log_utils
		log_utils.log('FuzzyBritches Addon Data Path Does not Exist. Creating Folder....', __name__, level=log_utils.LOGDEBUG)
		ad_folder = control.transPath('special://profile/addon_data/plugin.video.fuzzybritches')
		control.makeDirs(ad_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = control.addon('plugin.video.fuzzybritches').getAddonInfo('version')
...
def get_cache_version():
	...
	try:
		...
		from resources.lib.modules import log_utils
		log_utils.log('FuzzyBritches Addon Data Path Does not Exist. Creating Folder....', __name__, log_utils.LOGDEBUG)
		ad_folder = control.transPath('special://profile/addon_data/plugin.video.fuzzybritches')
		control.makeDirs(ad_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
