			...
			if action in self.selection_actions:
				...
				if focus_id == 2050: # listItems
					...
					for item in self.item_list:
						...
						self.selected_items.append(item.getProperty('fuzzybritches.trakt'))
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					if item.get('tvshowtitle'): listitem.setProperty('fuzzybritches.media_type', 'show')
					else: listitem.setProperty('fuzzybritches.media_type', 'movie')
