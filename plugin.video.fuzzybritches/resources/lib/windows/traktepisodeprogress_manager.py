			...
			if action in self.selection_actions:
				...
				if focus_id == 2060: # listItems
					...
					if chosen_listitem.getProperty('fuzzybritches.isSelected') == 'true':
						...
						season = chosen_listitem.getProperty('fuzzybritches.season')
						episode = chosen_listitem.getProperty('fuzzybritches.episode')
					...
					for item in self.item_list:
						...
						self.selected_items.append({'imdb': item.getProperty('fuzzybritches.imdb'), 'tvdb': item.getProperty('fuzzybritches.tvdb'),
							'season': item.getProperty('fuzzybritches.season'), 'episode': item.getProperty('fuzzybritches.episode')})
				...
				if cm_action == 'browseSeries':
					...
					self.execute_code('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches/?action=seasons&tvshowtitle=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&art=%s,return)' % (
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('fuzzybritches.tvshowtitle', tvshowtitle)
					...
					listitem.setProperty('fuzzybritches.premiered', new_date)
					season = str(item.get('season'))
					listitem.setProperty('fuzzybritches.season', season)
					episode = str(item.get('episode'))
					listitem.setProperty('fuzzybritches.episode', episode)
					label = '%s  -  %sx%s' % (tvshowtitle, season, episode.zfill(2))
					listitem.setProperty('fuzzybritches.label', label)
					...
					listitem.setProperty('fuzzybritches.tvdb', item.get('tvdb'))
					...
					listitem.setProperty('fuzzybritches.genre', str(item.get('genre', '')))
