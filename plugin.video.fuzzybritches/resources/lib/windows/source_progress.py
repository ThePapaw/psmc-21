# -*- coding: utf-8 -*-
"""
	FuzzyBritches Add-on (8/13/24) thanks kodifitzwell
"""
...
class WindowProgress(BaseDialog):
	...
	def set_controls(self):
		...
		if self.meta:
			if self.meta.get('clearlogo'): self.setProperty('fuzzybritches.clearlogo', self.meta.get('clearlogo'))
			if self.meta.get('fanart') and self.use_fanart: self.setProperty('fuzzybritches.fanart', self.meta.get('fanart'))
			else: self.setProperty('fuzzybritches.fanart', addonFanart())
			if self.meta.get('title'): self.setProperty('fuzzybritches.title', self.meta.get('title'))
			if 'tvshowtitle' in self.meta: self.setProperty('fuzzybritches.tvtitle', self.meta.get('tvshowtitle'))
			if self.meta.get('plot'): self.setProperty('fuzzybritches.plot', self.meta.get('plot', ''))
		else:
			self.setProperty('fuzzybritches.fanart', addonFanart())
		self.setProperty('fuzzybritches.icononly', self.icon_only)
		self.setProperty('fuzzybritches.isHidden', self.isHidden)
		self.setProperty('fuzzybritches.sources_highlight_color', self.source_color)
