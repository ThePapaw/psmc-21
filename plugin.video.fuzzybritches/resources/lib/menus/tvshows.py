...
class TVshows:
		...
		try:
			...
			if self.useContainerTitles:
				try: 
					...
					control.setHomeWindowProperty('fuzzybritches.tvrecent', str(getLS(40257)+' '+item.get('title')))
				except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
		...
		try:
			...
			if self.useContainerTitles:
				try: 
					...
					control.setHomeWindowProperty('fuzzybritches.tvsimilar', str(getLS(40257)+' '+item.get('title')))
				except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
	...
	def collectionManager(self):
		try:
			...
			del window
			if selected_items:
				# refresh = 'plugin.video.fuzzybritches' in control.infoLabel('Container.PluginName')
	...
	def search_new(self):
		...
		if self.traktCredentials:
			...
			url = self.search_tmdb_link % ('%s', quote_plus(q))
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches/?action=tvshows&url=%s,return)' % (quote_plus(url)))
	...
	def person(self, folderName=''):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches/?action=tvPersons&url=%s,return)' % (quote_plus(url)))
		...
		def userList_totalItems(url):
			...
			for item in items:
				try:
					...
					log_utils.error()
			return self.list
		_force_fresh = control.homeWindow.getProperty('fuzzybritches.trakt.userlist.modified') == 'true'
		if _force_fresh:
			control.homeWindow.clearProperty('fuzzybritches.trakt.userlist.modified')
		...
		try:
			...
			if getSetting('mdblist.paginate.lists') == 'true' and self.list:
				...
				try:
					...
					next_page = index + 2
					next = 'plugin://plugin.video.fuzzybritches/?action=mdbUserWatchListTVShows&url=%s&page=%s&folderName=%s' % (
		...
		try:
			...
			if getSetting('mdblist.paginate.lists') == 'true' and self.list:
				...
				try:
					if index + 1 >= total_pages: raise Exception()
					next_page = index + 2
					next = 'plugin://plugin.video.fuzzybritches/?action=mdbUserCollectionTVShows&url=%s&page=%s&folderName=%s' % (
		...
		try:
			...
			if self.list:
				...
				for i in self.list:
					try:
						...
						if is_widget and getSetting('enable.fuzzybritcheshidewatched') == 'true' and str(xbmc.getInfoLabel("Window.Property(xmlfile)")) != 'Custom_1114_Search.xml' and watched: continue
						filtered.append(i)
			...
			if getSetting('trakt.paginate.lists') == 'true' and self.list:
				...
				try:
					if index + 1 >= total_pages: raise Exception()
					next_page = index + 2
					next = 'plugin://plugin.video.fuzzybritches/?action=shows_progress&url=%s&page=%s&folderName=%s' % (
		...
		try:
			...
			if getSetting('mdblist.paginate.lists') == 'true' and self.list:
				...
				try:
					if index + 1 >= total_pages: raise Exception()
					next_page = index + 2
					next = 'plugin://plugin.video.fuzzybritches/?action=mdblist_shows_progress&url=%s&page=%s&folderName=%s' % (
...
#################################
	...
	def tvshowDirectory(self, items, next=True, isProgress=False, isWatched=False, isCollection=False, folderName='FuzzyBritches'):
		from sys import argv # some functions like ActivateWindow() throw invalid handle less this is imported here.
		if self.useContainerTitles: control.setContainerName(folderName)
		returnHome = control.folderPath()
		control.setHomeWindowProperty('fuzzybritches.returnhome', returnHome)
		...
		sysaddon, syshandle = 'plugin://plugin.video.fuzzybritches/', int(argv[1])
		is_widget = 'plugin' not in control.infoLabel('Container.PluginName')
...
####################################
	...
	def addDirectory(self, items, queue=False, folderName='FuzzyBritches'):
		from sys import argv # some functions like ActivateWindow() throw invalid handle less this is imported here.
		control.playlist.clear()
		if not items: # with reuselanguageinvoker on an empty directory must be loaded, do not use sys.exit()
			content = ''
