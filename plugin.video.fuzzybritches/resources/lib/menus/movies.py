...
class Movies:
		...
		try:
			...
			if getSetting('mdblist.paginate.lists') == 'true' and self.list:
				...
				try:
					...
					next_page = index + 2
					next = 'plugin://plugin.video.fuzzybritches/?action=mdbUserWatchListMovies&url=%s&page=%s&folderName=%s' % (
		...
		try:
			...
			if getSetting('mdblist.paginate.lists') == 'true' and self.list:
				...
				try:
					if index + 1 >= total_pages: raise Exception()
					next_page = index + 2
					next = 'plugin://plugin.video.fuzzybritches/?action=mdbUserCollectionMovies&url=%s&page=%s&folderName=%s' % (
	...
	def trakt_based_on_recent(self, create_directory=True, folderName=''):
		...
		try:
			...
			try: 
				...
				control.setHomeWindowProperty('fuzzybritches.movierecent', str(getLS(40257)+' '+item.get('title')))
			except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
	...
	def trakt_based_on_similar(self, create_directory=True, folderName=''):
		...
		try:
			...
			try: 
				...
				control.setHomeWindowProperty('fuzzybritches.moviesimilar', str(getLS(40259)+' '+item.get('title')))
			except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
	...
	def search_new(self):
		...
		if self.traktCredentials:
			...
			url = self.search_tmdb_link % ('%s', quote_plus(q))
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches/?action=movies&url=%s,return)' % (quote_plus(url)))
	...
	def person(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches/?action=moviePersons&url=%s,return)' % (quote_plus(url)))
	...
	def imdb_list(self, url, comingSoon=False, isRatinglink=False, folderName=''):
		from resources.lib.modules import log_utils
		log_utils.log('FuzzyBritches IMDB list generation. URL: %s'% str(url), 1)
		list = []
		try:
	...
	def imdb_genre_list(self, url, comingSoon=False, isRatinglink=False, folderName=''):
			from resources.lib.modules import log_utils
			log_utils.log('FuzzyBritches Genre IMDB list generation. URL: %s'% str(url), 1)
			list = []
			try:
				for i in re.findall(r'date\[(\d+)\]', url):
					url = url.replace('date[%s]' % i, (self.date_time - timedelta(days=int(i))).strftime('%Y-%m-%d'))
	...
	def similarFromLibrary(self, tmdb=None, create_directory=True, folderName=''):
		try:
			...
			if not original_movie:
				...
				control.setHomeWindowProperty('fuzzybritches.moviesimilarlibrary', str(getLS(40257)+' '+original_movie["title"]))
...
###########################################################
	...
	def addDirectory(self, items, queue=False, folderName=''):
		...
		homeWindow.setProperty('fuzzybritches.returnhome', returnHome)
		if not items: # with reuselanguageinvoker on an empty directory must be loaded, do not use sys.exit()
			content = ''
			control.hide()
			if is_widget != True:
