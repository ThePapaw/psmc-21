...
class Navigator:
	...
	def _browseForIcon(art_path):
		import xbmcgui
		_SKIN = 'special://skin/'
		choice = control.selectDialog(['Browse for Image', 'Enter URL', 'FuzzyBritches Icons'], heading='Select Icon')
		if choice < 0:
			return None
		if choice == 0:
	...
	def customFolderManager(self):
		from resources.lib.database import menu as menu_db
		menu_db.initialize('root')
		addon_url = 'plugin://plugin.video.fuzzybritches/?action=customFolderNavigator&folder_id='
		while True:
			folders = menu_db.get_custom_folders()
	...
	def tools(self, folderName=''):
		...
		#self.addDirectoryItem(32506, 'tools_contextFuzzyBritchesSettings', 'icon.png', 'DefaultAddonProgram.png', isFolder=False)
		#-- Providers - 
		...
		try:
			...
			if multi_context:
				for ctx_label, ctx_action in multi_context:
					cm.append((ctx_label, 'RunPlugin(plugin://plugin.video.fuzzybritches/?action=%s)' % ctx_action))
