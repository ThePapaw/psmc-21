...
class Navigator:
	...
	def root(self):
		...
		if getMenuEnabled('navi.changelog'): self.addDirectoryItem(32014, 'tools_ShowChangelog&name=FuzzyBritches', 'changelog.png', 'DefaultAddonHelper.png', isFolder=False)
		if getMenuEnabled('navi.fullchangelog'): self.addDirectoryItem(40589, 'tools_ShowFullChangelog&name=FuzzyBritches', 'changelog.png', 'DefaultAddonHelper.png', isFolder=False)
		self.endDirectory()
	...
	def traktSearchLists(self, media_type):
		...
		control.closeAll()
		if media_type == 'movies': control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches/?action=movies_PublicLists&url=%s,return)' % (quote_plus(url)))
		else: control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches/?action=tv_PublicLists&url=%s,return)' % (quote_plus(url)))
	...
	def tools(self, folderName=''):
		...
		#self.addDirectoryItem(32506, 'tools_contextFuzzyBritchesSettings', 'icon.gif', 'DefaultAddonProgram.png', isFolder=False)
		#-- Providers - 
	...
	def loggingNavigator(self, folderName=''):
		if self.useContainerTitles: control.setContainerName(folderName)
		self.addDirectoryItem(32524, 'tools_viewLogFile&name=FuzzyBritches', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32525, 'tools_clearLogFile', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32526, 'tools_ShowChangelog&name=FuzzyBritches', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32529, 'tools_ShowFullChangelog&name=FuzzyBritches', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32527, 'tools_uploadLogFile&name=FuzzyBritches', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
	...
	# 		title = getLS(32059)
	# 		url = 'plugin://plugin.video.fuzzybritches/?action=tools_addView&content=%s' % content
	...
	def addDirectoryItem(self, name, query, poster, icon, context=None, queue=False, isAction=True, isFolder=True, isPlayable=False, isSearch=False, table=''):
		try:
			...
			url = 'plugin://plugin.video.fuzzybritches/?action=%s' % query if isAction else query
			...
			if queue: cm.append((queueMenu, 'RunPlugin(plugin://plugin.video.fuzzybritches/?action=playlist_QueueItem)'))
			if context: cm.append((getLS(context[0]), 'RunPlugin(plugin://plugin.video.fuzzybritches/?action=%s)' % context[1]))
			if isSearch: cm.append(('Clear Search Phrase', 'RunPlugin(plugin://plugin.video.fuzzybritches/?action=cache_clearSearchPhrase&source=%s&name=%s)' % (table, quote_plus(name))))
			cm.append(('[COLOR %s]FuzzyBritches Settings[/COLOR]' % self.highlight_color, 'RunPlugin(plugin://plugin.video.fuzzybritches/?action=tools_openSettings)'))
