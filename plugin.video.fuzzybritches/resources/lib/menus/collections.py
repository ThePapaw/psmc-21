...
class Collections:
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches/?action=collections&url=%s,return)' % (quote_plus(url)))
...
####################################
	...
	def addDirectoryItem(self, name, action, poster, icon, fanart=None, context=None, queue=False, folderName=''):
		try:
			...
			url = 'plugin://plugin.video.fuzzybritches/?action=%s' % action
			cm = []
