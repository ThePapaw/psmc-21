...
class TorBox:
	...
	def auth(self):
		...
		line = '%s\n%s\n%s'
		try:
			response = requests.get('%s/user/auth/device/start?app=FuzzyBritches' % base_url, timeout=self.timeout)
			...
			control.openSettings('6.6', 'plugin.video.fuzzybritches')
			return True
		except:
	...
	def referral_link(self):
		...
		try:
			...
			line = '%s\n%s' % ('[COLOR %s][B]FuzzyBritches TorBox Referral Link[/B][/COLOR]' % highlight_color, referral_url)
