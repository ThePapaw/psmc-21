	...
	def run(self):
		...
		try:
			...
			if len(local_version) > 6: #test version
				repo_xml = requests.get('https://raw.githubusercontent.com/fuzzybritcheskodi/fuzzybritcheskodi/master/matrix/plugin.video.fuzzybritches/addon.xml')
			else:
...
class SyncTraktService:
	def run(self):
		...
		control.log('[ plugin.video.fuzzybritches ]  Trakt Sync Service Starting (sync check every %s minutes)...' % service_syncInterval, LOGINFO)
...
def main():
	...
	control.log('[ plugin.video.fuzzybritches ]  Trakt Sync Service Stopping...', LOGINFO)
	if libraryService:
