...
properties = [
	...
	'context.fuzzybritches.artworkCustomize',
	...
	'context.fuzzybritches.libMdblistManager',
	'context.fuzzybritches.addLibtoFavourite']

class CheckSettingsFile:
	def run(self):
		try:
			control.log('[ plugin.video.fuzzybritches ]  CheckSettingsFile Service Starting...', LOGINFO)
			...
			return control.log('[ plugin.video.fuzzybritches ]  Finished CheckSettingsFile Service', LOGINFO)
		except Exception:
			log_utils.error()
	...
	def __init__ (self):
		...
		for id in properties:
			if control.setting(id) == 'true':
				...
				#xbmc.log('[ plugin.video.fuzzybritches.context ]  menu item enabled: {0}'.format(id), LOGINFO)
		control.log('[ plugin.video.fuzzybritches ]  Settings Monitor Service Starting...', LOGINFO)

	def onSettingsChanged(self):
		if window.getProperty('fuzzybritches.updateSettings') != 'true':
			return control.log('[ plugin.video.fuzzybritches ]  Settings Monitor is off.', LOGINFO)
		...
		try:
			...
			control.log('[ plugin.video.fuzzybritches ]  Exception clearing settings property...', LOGDEBUG)

		try:
			control.refresh_playAction()
		except Exception:
			control.log('[ plugin.video.fuzzybritches ]  Exception making refreshing playAction...', LOGDEBUG)
		try:
			control.refresh_libPath()
		except Exception:
			control.log('[ plugin.video.fuzzybritches ]  Exception refreshing libpath...', LOGDEBUG)
		#try:
			#control.checkPlayNextEpisodes()
		#except:
			#control.log('[ plugin.video.fuzzybritches ]  Exception checking playnext episodes...', LOGDEBUG)
		try:
			control.refresh_debugReversed()
		except Exception:
			control.log('[ plugin.video.fuzzybritches ]  Exception checking debug reversed', LOGDEBUG)
		try:
			control.setContextColors()
		except Exception:
			control.log('[ plugin.video.fuzzybritches ]  Exception setting context colors...', LOGDEBUG)
		try:
			control.checkModules()
		except Exception:
			control.log('[ plugin.video.fuzzybritches ]  Exception checking modules...', LOGDEBUG)
		try:
			control.sleep(50)
			control.make_settings_dict()
			window.setProperty('fuzzybritches.updateSettings','true')
		except Exception:
			control.log('[ plugin.video.fuzzybritches ]  Exception making settings dict...', LOGDEBUG)
		try:
			for id in properties:
				if control.setting(id) == 'true':
					window.setProperty(id, 'true')
					...
					#xbmc.log('[ plugin.video.fuzzybritches.context ]  menu item disabled: {0}'.format(id), LOGINFO)
		except Exception:
			log_utils.error()

class SyncMyAccounts:
	def run(self):
		control.log('[ plugin.video.fuzzybritches ]  Sync Accounts with Scraper Starting...', LOGINFO)
		control.syncAccounts()
		return control.log('[ plugin.video.fuzzybritches ]  Finished Sync with Scraper', LOGINFO)

class SyncMovieLibrary:
	def run(self):
		if control.setting('library.cachesimilar') == 'true':
			control.log('[ plugin.video.fuzzybritches ]  Sync Library Movies with FuzzyBritches...', LOGINFO)
			from resources.lib.modules import library
			library.lib_tools().cacheLibraryforSimilar() 
			return control.log('[ plugin.video.fuzzybritches ]  Sync Library Movies with FuzzyBritches Done', LOGINFO)
		else:
			return

class checkAutoStart:
	def run(self):
		control.log('[ plugin.video.fuzzybritches ]  Checking for AutoStart....', LOGINFO)
		if control.setting('fuzzybritches.autostart') == 'true': 
			control.execute('RunAddon(plugin.video.fuzzybritches)')
		return control.log('[ plugin.video.fuzzybritches ]  Finished AutoStart Check', LOGINFO)


class ReuseLanguageInvokerCheck:
	def run(self):
		control.log('[ plugin.video.fuzzybritches ]  ReuseLanguageInvokerCheck Service Starting...', LOGINFO)
		try:
			import xml.etree.ElementTree as ET
			from resources.lib.modules.language_invoker import gen_file_hash
			addon_xml = control.joinPath(control.addonPath('plugin.video.fuzzybritches'), 'addon.xml')
			tree = ET.parse(addon_xml)
			root = tree.getroot()
			current_addon_setting = control.addon('plugin.video.fuzzybritches').getSetting('reuse.languageinvoker')
			try: current_xml_setting = [str(i.text) for i in root.iter('reuselanguageinvoker')][0]
			except Exception: return control.log('[ plugin.video.fuzzybritches ]  ReuseLanguageInvokerCheck failed to get settings.xml value', LOGINFO)
			...
			if current_xml_setting == current_addon_setting:
				return control.log('[ plugin.video.fuzzybritches ]  ReuseLanguageInvokerCheck Service Finished', LOGINFO)
			...
			for item in root.iter('reuselanguageinvoker'):
				...
				control.log('[ plugin.video.fuzzybritches ]  ReuseLanguageInvokerCheck Service Finished', LOGINFO)
...
class AddonCheckUpdate:
	def run(self):
		control.log('[ plugin.video.fuzzybritches ]  Addon checking available updates', LOGINFO)
		try:
			import re
			import requests
			...
			if len(local_version) > 6: #test version
				repo_xml = requests.get('https://raw.githubusercontent.com/fuzzybritcheskodi/fuzzybritcheskodi.github.io/master/matrix/plugin.video.fuzzybritches/addon.xml')
			else:
				repo_xml = requests.get('https://raw.githubusercontent.com/fuzzybritchesplug/fuzzybritchesplug.github.io/master/matrix/plugin.video.fuzzybritches/addon.xml')
			if not repo_xml.status_code == 200:
				return control.log('[ plugin.video.fuzzybritches ]  Could not connect to remote repo XML: status code = %s' % repo_xml.status_code, LOGINFO)
			repo_version = re.findall(r'<addon id=\"plugin.video.fuzzybritches\".+version=\"(\d*.\d*.\d*)\"', repo_xml.text)[0]
			...
			if check_version_numbers(local_version, repo_version):
				while control.condVisibility('Library.IsScanningVideo'):
					control.sleep(10000)
				control.log('[ plugin.video.fuzzybritches ]  A newer version is available. Installed Version: v%s, Repo Version: v%s' % (local_version, repo_version), LOGINFO)
				control.notification(message=control.lang(35523) % repo_version)
			return control.log('[ plugin.video.fuzzybritches ]  Addon update check complete', LOGINFO)
		except Exception:
			log_utils.error()
			...
			if isUpdate:
				...
				if do_cacheClear:
					...
					if cleared:
						...
						control.log('[ plugin.video.fuzzybritches ]  Forced traktsync clear for version update complete.', LOGINFO)
					if cleared2:
						control.notification(message='Forced simklsync clear for version update complete.')
						control.log('[ plugin.video.fuzzybritches ]  Forced simklsync clear for version update complete.', LOGINFO)
					if clr_fanarttv:
						...
						control.log('[ plugin.video.fuzzybritches ]  Forced fanarttv.db clear for version update complete.', LOGINFO)
				control.setSetting('trakt.message2', '') # force a settings write for any added settings that may have been added in new version
				control.log('[ plugin.video.fuzzybritches ]  Forced new User Data settings.xml saved', LOGINFO)
				control.log('[ plugin.video.fuzzybritches ]  Plugin updated to v%s' % curVersion, LOGINFO)
		except Exception:
			log_utils.error()

class SyncTraktCollection:
	def run(self):
		control.log('[ plugin.video.fuzzybritches ]  Trakt Collection Sync Import Disabled...', LOGINFO)
		#control.log('[ plugin.video.fuzzybritches ]  Trakt Collection Sync Starting...', LOGINFO)
		#control.execute('RunPlugin(%s?action=library_tvshowsToLibrarySilent&url=traktcollection)' % plugin)
		#control.log('[ plugin.video.fuzzybritches ]  Trakt Collection Sync TV Shows Complete', LOGINFO)
		#control.execute('RunPlugin(%s?action=library_moviesToLibrarySilent&url=traktcollection)' % plugin)
		#control.log('[ plugin.video.fuzzybritches ]  Trakt Collection Sync Movies Complete', LOGINFO)
		#control.log('[ plugin.video.fuzzybritches ]  Trakt Collection Sync Complete', LOGINFO)
...
class LibraryService:
	def run(self):
		try:
			...
			library_hours = int(6)
		control.log('[ plugin.video.fuzzybritches ]  Library Update Service Starting (Runs Every %s Hours)...' % library_hours,  LOGINFO)
		from resources.lib.modules import library
...
class SyncServices:
	def run(self):
		...
		control.log('[ plugin.video.fuzzybritches ]  Account Sync Service Starting (sync check every %s minutes)...' % service_syncInterval, LOGINFO)
...
try:
	...
	if len(str(control.getFuzzyBritchesVersion())) > 6:
		repoVersion = control.addon('repository.fuzzybritcheskodi').getAddonInfo('version')
		repoName = 'repository.fuzzybritcheskodi'
	...
	log_utils.log('##   plugin.video.fuzzybritches Version: %s' % str(addonVersion), level=LOGINFO)
...
try:
	...
	log_utils.log('########   FuzzyBritches SERVICE CONFIGURATION   ########', level=LOGINFO)
...
class PremAccntNotification:
	def run(self):
		...
		control.log('[ plugin.video.fuzzybritches ] Debrid Account Expiry Notification Service Starting...', LOGINFO)
...
def main():
	while not control.monitor.abortRequested():
		control.log('[ plugin.video.fuzzybritches ]  Service Started', LOGINFO)
		schedTrakt = None
		libraryService = None
	...
	control.log('[ plugin.video.fuzzybritches ]  Settings Monitor Service Stopping...', LOGINFO)
	syncServices.join(2)
	control.log('[ plugin.video.fuzzybritches ]  Account Sync Service Stopping...', LOGINFO)
	if libraryService:
		libraryService.join(2)
		control.log('[ plugin.video.fuzzybritches ]  Library Update Service Stopping...', LOGINFO)
	if schedTrakt:
		schedTrakt.cancel()
	control.log('[ plugin.video.fuzzybritches ]  Service Stopped', LOGINFO)
