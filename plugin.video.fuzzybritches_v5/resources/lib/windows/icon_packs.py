...
class IconPacksView(BaseDialog):
		...
		try:
			if action in self.selection_actions:
				...
				if focus_id == 2050: # listItems
					...
					packname = chosen_listitem.getProperty('fuzzybritches.title')
					downloaded = chosen_listitem.getProperty('fuzzybritches.downloaded')
					url = chosen_listitem.getProperty('fuzzybritches.downloadurl')
					active = chosen_listitem.getProperty('fuzzybritches.active')
					poster = chosen_listitem.getProperty('fuzzybritches.imageurl')
					if downloaded == '1' and active == '0' and str(packname).lower() != 'fuzzybritches':
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('fuzzybritches.title', item.get('name'))
					listitem.setProperty('fuzzybritches.isSelected', '')
					poster = item.get('imageurl', '')
					listitem.setProperty('fuzzybritches.poster', poster)
					listitem.setProperty('fuzzybritches.downloadurl', item.get('url'))
					listitem.setProperty('fuzzybritches.downloaded', item.get('downloaded'))
					if str(item.get('name')).lower() == self.activePack:
						listitem.setProperty('fuzzybritches.active', '1')
					else:
						listitem.setProperty('fuzzybritches.active', '0')
					yield listitem
				except:
					from resources.lib.modules import log_utils
					log_utils.error()
	...
	def set_properties(self):
		try:
			...
			self.setProperty('fuzzybritches.fanart', self.fanart)
		except:
			from resources.lib.modules import log_utils
			log_utils.error()