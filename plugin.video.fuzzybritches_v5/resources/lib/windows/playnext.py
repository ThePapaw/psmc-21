...
class PlayNextXML(BaseDialog):
	...
	def onInit(self):
		playerWindow.setProperty('fuzzybritches.playnextPlayPressed', str(0))
		self.set_properties()
	...
	def onClick(self, control_id):
		if control_id == 3011: # Play Now, skip to end of current
			playerWindow.setProperty('fuzzybritches.playnextPlayPressed', str(1))
	...
	def set_properties(self):
		...
		try:
			...
			self.setProperty('fuzzybritches.tvshowtitle', self.meta.get('tvshowtitle'))
			...
			self.setProperty('fuzzybritches.title', self.meta.get('title'))
			self.setProperty('fuzzybritches.year', str(self.meta.get('year', '')))
			...
			self.setProperty('fuzzybritches.rating', str(self.meta.get('rating', '')))
			self.setProperty('fuzzybritches.landscape', self.meta.get('landscape', ''))
			self.setProperty('fuzzybritches.fanart', self.meta.get('fanart', ''))
			self.setProperty('fuzzybritches.thumb', self.meta.get('thumb', ''))
			self.setProperty('fuzzybritches.episode_type', self.meta.get('episode_type'))
			try:
				next_duration = int(self.meta.get('duration')) if self.meta.get('duration') else ''
				self.setProperty('fuzzybritches.duration', str(int(next_duration)))
				endtime = (datetime.now() + timedelta(seconds=next_duration)).strftime('%I:%M %p').lstrip('0') if next_duration else ''
				self.setProperty('fuzzybritches.endtime', endtime)
			except:
				self.setProperty('fuzzybritches.duration', '')
				self.setProperty('fuzzybritches.endtime', '')
			self.setProperty('fuzzybritches.playnext.background.color', self.playNextBG)
			if getSetting('playnext.hidebutton') == 'false':
				self.setProperty('fuzzybritches.hidebutton','true')
