		...
		try:
			...
			if action_id in self.selection_actions:
				...
				if cm_action == 'sourceInfo':
					...
					self.execute_code('RunPlugin(plugin://plugin.video.fuzzybritches_v5/?action=showDebridPack&caller=%s&name=%s&url=%s&source=%s)' %
					...
					self.execute_code('RunPlugin(plugin://plugin.video.fuzzybritches_v5/?action=download&name=%s&image=%s&source=%s&caller=sources&title=%s)' %
					...
					self.execute_code('RunPlugin(plugin://plugin.video.fuzzybritches_v5/?action=createStrm&name=%s&image=%s&source=%s&caller=sources&title=%s)' %
										(new_sysname, quote_plus(poster), quote_plus(source_dict), sysname))
					self.selected = (None, '')
