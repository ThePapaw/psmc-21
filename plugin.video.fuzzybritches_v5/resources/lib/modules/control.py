...
def getProgressWindow(heading='', icon=None, qr=0, artwork=0):
	if icon == None:
		icon = addonIcon()
	from resources.lib.windows.fuzzybritches_dialogs import ProgressFuzzyBritches
	window = ProgressFuzzyBritches('progress_fuzzybritches.xml', addonPath(addonId()), heading=heading, icon=icon, qr=qr, artwork=artwork)
	Thread(target=window.run).start()
	return window

def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('fuzzybritches_settings'))
...
def getFuzzyBritchesVersion():
	return xbmcaddon.Addon('plugin.video.fuzzybritches_v5').getAddonInfo('version')
...
def artPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.fuzzybritches_v5').getAddonInfo('path'), 'resources', 'artwork', theme)

def genreIconPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.fuzzybritches_v5').getAddonInfo('path'), 'resources', 'artwork', theme, 'genre_media', 'icons')

def genrePosterPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.fuzzybritches_v5').getAddonInfo('path'), 'resources', 'artwork', theme, 'genre_media', 'posters')
...
def iconFolders():
	return joinPath(xbmcaddon.Addon('plugin.video.fuzzybritches_v5').getAddonInfo('path'), 'resources', 'artwork')
...
def skin_location():
	return transPath('special://home/addons/plugin.video.fuzzybritches_v5')
...
def yesnoDialog(line1, line2, line3, heading=addonInfo('name'), nolabel='', yeslabel='', icon=None):
	...
	if setting('dialogs.usefuzzybritchesdialog') == 'true':
		from resources.lib.windows.fuzzybritches_dialogs import Confirm
		if yeslabel == '':
			myyes = lang(32179)
		else:
			myyes = yeslabel
...
def okDialog(title=None, message=None, icon=None):
	...
	if setting('dialogs.usefuzzybritchesdialog') == 'true':
		if icon == None:
			icon = addonIcon()
		from resources.lib.windows.fuzzybritches_dialogs import OK
...
def refresh_playAction(): # for fuzzybritches global CM play actions
	...
	homeWindow.setProperty('fuzzybritches.autoPlaytv.enabled', autoPlayTV)
	autoPlayMovie = 'true' if setting('play.mode.movie') == '1' else '0'
	homeWindow.setProperty('fuzzybritches.autoPlayMovie.enabled', autoPlayMovie)

def refresh_libPath(): # for fuzzybritches global CM library actions
	homeWindow.setProperty('fuzzybritches.movieLib.path', transPath(setting('library.movie')))
	homeWindow.setProperty('fuzzybritches.tvLib.path', transPath(setting('library.tv')))

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear fuzzybritches.log if setting to reverse has been changed
	if homeWindow.getProperty('fuzzybritches.debug.reversed') != setting('debug.reversed'):
...
def disable_enable_addon():
    ...
    jsonrpc('{"jsonrpc": "2.0", "id":1, "method": "Addons.SetAddonEnabled", "params": {"addonid": "plugin.video.fuzzybritches_v5", "enabled": false }}')
    xbmc.log('[ plugin.video.fuzzybritches_v5 ] fuzzybritches disabled', 1)
    jsonrpc('{"jsonrpc": "2.0", "id":1, "method": "Addons.SetAddonEnabled", "params": {"addonid": "plugin.video.fuzzybritches_v5", "enabled": true }}')
    xbmc.log('[ plugin.video.fuzzybritches_v5 ] fuzzybritches re-enabled', 1)
...
def syncAccounts():
	try:
		if setting('fuzzybritches.colorSecond') == 'false':
			homeWindow.setProperty('fuzzybritches.updateSettings', 'false')
			...
			homeWindow.setProperty('fuzzybritches.updateSettings', 'true')
			setSetting('fuzzybritches.colorSecond', 'true')
			notification('FuzzyBritches', 'Reloading addon due to new settings added.')
		if setting('fuzzybritches.externalWarning') != 'true':
			setSetting('fuzzybritches.externalWarning', 'true')
			from resources.help import help
			help.get('externalProviders')
		if setting('context.useFuzzyBritchesContext') == 'true':
			homeWindow.setProperty('context.fuzzybritches.showFuzzyBritches', '[B][COLOR '+setting('highlight.color')+']FuzzyBritches[/COLOR][/B] - ')
		else:
			homeWindow.setProperty('context.fuzzybritches.showFuzzyBritches', '')
		homeWindow.setProperty('context.fuzzybritches.highlightcolor', setting('highlight.color'))
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
...
def backToMain(folder):
	if folder == 'movies':
		url = 'plugin://plugin.video.fuzzybritches_v5/?action=movieNavigator&folderName=Discover%20Movies'
	elif folder =='tvshows':
		url = 'plugin://plugin.video.fuzzybritches_v5/?action=tvNavigator&folderName=Discover%20TV%20Shows'
	else:
		url = None
