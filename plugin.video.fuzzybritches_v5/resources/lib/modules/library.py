	...
	def importListsManager(self, fromSettings=False):
		try:
			...
			if fromSettings == True:
				control.openSettings('13.2', 'plugin.video.fuzzybritches_v5')
...
class libmovies:
	...
	def auto_movie_setup(self):
		try:
			...
			library_sources.add_source('FuzzyBritches Movies', self.library_folder, source_content, 'DefaultMovies.png')
		except: log_utils.error()
	...
	def strmFile(self, i):
		try:
			...
			if control.setting('library.strm.use_tmdbhelper') == 'true':
				...
				content = 'plugin://plugin.video.fuzzybritches_v5/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s' % (systitle, year, imdb, tmdb)
...
class libtvshows:
	...
	def auto_tv_setup(self):
		try:
			...
			source_name = 'FuzzyBritches TV Shows'
			# TVDb scraper
	...
	def strmFile(self, i):
		try:
			...
			if control.setting('library.strm.use_tmdbhelper') == 'true':
				...
				content = 'plugin://plugin.video.fuzzybritches_v5/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s' % (
		...
		try:
			...
			for file in episode:
				try:
					...
					read = file.read()
					file.close()
					if not read.startswith(('plugin://plugin.video.themoviedb.helper', 'plugin://plugin.video.fuzzybritches_v5')): continue
