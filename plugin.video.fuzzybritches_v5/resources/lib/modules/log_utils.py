...
DEBUGPREFIX = '[ plugin.video.fuzzybritches_v5: %s ]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ plugin.video.fuzzybritches_v5 ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		if exception:
			...
			addon = 'plugin.video.fuzzybritches_v5'
		...
		import xbmc
		xbmc.log('[ plugin.video.fuzzybritches_v5 ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)
...
def clear_logFile():
	...
	try:
		...
		import xbmc
		xbmc.log('[ plugin.video.fuzzybritches_v5 ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared
...
def upload_LogFile(name):
	...
	try:
		...
		text = f.read()
		f.close()
		UserAgent = 'FuzzyBritches %s' % addonVersion('plugin.video.fuzzybritches_v5')
