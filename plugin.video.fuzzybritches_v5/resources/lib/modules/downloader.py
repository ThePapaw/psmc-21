...
def download(name, image, url, meta_name=None, pack=None): # needs re-write, pack file support
	...
	try:
		...
		if len(content) == 0:
			...
			if not dest: dest = 'special://profile/addon_data/plugin.video.fuzzybritches_v5/Movies/'
			for level in levels:
			...
			if not dest: dest = 'special://profile/addon_data/plugin.video.fuzzybritches_v5/TVShows/'
			for level in levels:
				try: control.makeFile(os.path.abspath(os.path.join(dest, level)))
				except:
					log_utils.error()
