...
def traktAuth(fromSettings=0):
	...
	try:
		...
		if deviceCode:
			...
			if fromSettings == 1:
				control.openSettings('8.0', 'plugin.video.fuzzybritches_v5')
			return True
		if fromSettings == 1:
				control.openSettings('8.0', 'plugin.video.fuzzybritches_v5')
...
def unHideItems(tvdb_ids):
	...
	try:
		...
		for section in sections:
			...
			control.sleep(1000)
		if success:
			if 'plugin.video.fuzzybritches_v5' in control.infoLabel('Container.PluginName'): control.refresh()
	...
	try:
		...
		if select >= 0:
			if items[select][1] == 'watch':
				...
				control.execute('RunPlugin(plugin://plugin.video.fuzzybritches_v5/?action=shows_traktHiddenManager)')
			elif items[select][1] == 'unfinishedEpisodeManager':
				control.execute('RunPlugin(plugin://plugin.video.fuzzybritches_v5/?action=episodes_traktUnfinishedManager)')
			elif items[select][1] == 'unfinishedMovieManager':
				control.execute('RunPlugin(plugin://plugin.video.fuzzybritches_v5/?action=movies_traktUnfinishedManager)')
