...
class Player(xbmc.Player):
	def __init__(self):
		...
		self.multi_season = getSetting('fuzzybritches.multiSeason') == 'true'
...
##################
	...
	def keepAlive(self):
		...
		if playerWindow.getProperty('fuzzybritches.playlistStart_position'): pass
		else:
			if control.playlist.size() > 1: playerWindow.setProperty('fuzzybritches.playlistStart_position', str(control.playlist.getposition()))
						...
						if self.enable_playnext and not self.play_next_triggered:
							if int(control.playlist.size()) > 1:
								if self.preScrape_triggered == False:
									xbmc.executebuiltin('RunPlugin(plugin://plugin.video.fuzzybritches_v5/?action=play_preScrapeNext)')
								...
								if self.playnext_method== '0':
									if remaining_time < (self.playnext_time + 1) and remaining_time != 0:
										...
										xbmc.executebuiltin('RunPlugin(plugin://plugin.video.fuzzybritches_v5/?action=play_nextWindowXML)')
...
class PlayNext(xbmc.Player):
	...
	def isStill_watching(self):
		...
		playlistStart_position = playerWindow.getProperty('fuzzybritches.playlistStart_position')
	...
	def getNext_meta(self):
		try:
			...
			elif 'plugin.video.fuzzybritches_v5' in next_url:
