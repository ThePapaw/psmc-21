...
def router(argv2):
	...
	if action is None:
		...
		if isUpdate == 'true':
			control.execute('RunPlugin(plugin://plugin.video.fuzzybritches_v5/?action=tools_cleanSettings)')
	...
	####################################################
		...
		if action == 'play_Item':
			...
			for i in items:
				...
				url = 'plugin://plugin.video.fuzzybritches_v5/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select=1' % (
			...
			if rtype == 'movie':
				...
				r = 'plugin://plugin.video.fuzzybritches_v5/?action=play_Item'
			elif rtype == 'episode':
				from resources.lib.menus import episodes
				...
				r = 'plugin://plugin.video.fuzzybritches_v5/?action=play_Random&rtype=episode'
			elif rtype == 'show':
				from resources.lib.menus import tvshows
				rlist = tvshows.TVshows().get(url, create_directory=False)
				r = 'plugin://plugin.video.fuzzybritches_v5/?action=play_Random&rtype=season'
	...
	####################################################
		...
		control.openSettings('0.2', 'plugin.video.fuzzybritches_v5')
