...
class TVshows:
	...
	def search_new(self):
		...
		if self.traktCredentials:
			...
			url = self.search_tmdb_link % ('%s', quote_plus(q))
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches_v5/?action=tvshows&url=%s,return)' % (quote_plus(url)))
	...
	def person(self, folderName=''):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches_v5/?action=tvPersons&url=%s,return)' % (quote_plus(url)))
...
#################################
	...
	def tvshowDirectory(self, items, next=True, isProgress=False, isWatched=False, folderName='FuzzyBritches'):
