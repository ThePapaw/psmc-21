...
class Navigator:
	...
	def traktSearchLists(self, media_type):
		...
		control.closeAll()
		if media_type == 'movies': control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches_v5/?action=movies_PublicLists&url=%s,return)' % (quote_plus(url)))
		else: control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches_v5/?action=tv_PublicLists&url=%s,return)' % (quote_plus(url)))
	...
	def views(self, folderName=''):
		try:
			...
			title = getLS(32059)
			url = 'plugin://plugin.video.fuzzybritches_v5/?action=tools_addView&content=%s' % content
	...
	def addDirectoryItem(self, name, query, poster, icon, context=None, queue=False, isAction=True, isFolder=True, isPlayable=False, isSearch=False, table=''):
		try:
			...
			url = 'plugin://plugin.video.fuzzybritches_v5/?action=%s' % query if isAction else query
			...
			if isSearch: cm.append(('Clear Search Phrase', 'RunPlugin(plugin://plugin.video.fuzzybritches_v5/?action=cache_clearSearchPhrase&source=%s&name=%s)' % (table, quote_plus(name))))
