...
class Movies:
	...
	def unfinishedManager(self):
		try:
			...
			del window
			if selected_items:
				refresh = 'plugin.video.fuzzybritches_v5' in control.infoLabel('Container.PluginName')
	...
	def search_new(self):
		...
		if self.traktCredentials:
			...
			url = self.search_tmdb_link % ('%s', quote_plus(q))
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches_v5/?action=movies&url=%s,return)' % (quote_plus(url)))
	...
	def person(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.fuzzybritches_v5/?action=moviePersons&url=%s,return)' % (quote_plus(url)))
	...
	def similarFromLibrary(self, tmdb=None, create_directory=True, folderName=''):
		...
		try:
			...
			all_titles = list()
			if originalMovie:
				#log_utils.log('[plugin.video.fuzzybritches_v5] Original Movie properties %s.'% str(originalMovie),1)
			...
			if originalMovie:
				try: 
					...
					control.setHomeWindowProperty('fuzzybritches.moviesimilarlibrary', str(getLS(40257)+' '+originalMovie["title"]))
				except: pass
			if self.list is None: self.list = []
			is_widget = 'plugin' not in control.infoLabel('Container.PluginName')
			if create_directory: self.movieDirectory(self.list, folderName=folderName)
...
#################################
	...
	def movieDirectory(self, items, unfinished=False, next=True, folderName=''):
		...
		sysaddon, syshandle = 'plugin://plugin.video.fuzzybritches_v5/', int(argv[1])
